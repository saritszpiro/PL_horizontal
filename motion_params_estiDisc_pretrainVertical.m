...
%      stimuli params 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
stim = struct('dur', {0.8}, 'possibleAngels', {[100 80 310 260]},'boundaryAngle', {[90 90 270 270]},...
    'radiusDeg',{8}, 'bkColor', {gray}, 'coh', {0.25}, 'speedDegPerSec', {10},'lifetime', {1}, 'limitLifetime', {0.05});

%cw/ccw: Note, response is encoded in responseVar.cw_ccw = [1 2]. Any changes must be done there as well
stim.cw_ccw = [2 1 1 2]; %CW (1) or CCW (2) from the boundary, for example 3deg is CCW from 0deg
...
responseVar = struct( 'allowedRespKeys', {['g', 'f']}, 'dur',{5},'keyEscape', 'ESCAPE', 'percentEstimation', {0});
