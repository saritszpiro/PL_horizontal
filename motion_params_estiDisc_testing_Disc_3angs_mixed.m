...
screenVar = struct('num', {max(Screen('Screens'))}, 'rectPix',{[0 0  1280 960]}, 'dist', {57}, 'size', {[40 30]},...
                    'res', {[1280 960 ]}, 'calib_filename', {'0001_james_110412_ProP220f_1280x960_100Hz_57cm_110412.mat'}); 
...
%      stimuli params 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
stim = struct('dur', {.2}, 'possibleAngels', {[4 356; 2 358; 8 352]},'boundaryAngle', {[0 0]},...
...
stim.cw_ccw = [2 1; 2 1; 2 1]; %CW (1) or CCW (2) from the boundary, for example 3deg is CCW from 0deg
...
responseVar = struct( 'allowedRespKeys', {['6', '3']}, 'dur',{.9},'keyEscape', 'ESCAPE', 'percentEstimation', {0});
...
%      Block params
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
blockVars = struct('numBlocks', 2);
if stim.angsBlocked
    if size(stim.possibleAngels,1)>1
        if blockVars.numBlocks ~= size(stim.possibleAngels,1)
            error('Number of blocks should match angle pairs');
        end
    end
end
...
if mod(trialVars.numTrialsPerBlock,size(stim.possibleAngels,1)*size(stim.possibleAngels,2))~=0
